...
wide <- dyad_df %>% 
  dplyr::select(Participant_ID, dayR, first_similarNC, genderN, sloppy_roommateC, awish) %>% 
  tidyr::pivot_wider(names_from = dayR,
              values_from = c(awish,
                              sloppy_roommateC,
                              first_similarNC))

readr::write_csv(wide, "wide sample data_two_level.csv",  na = "")
```
...
read.csv("wide sample data_two_level.csv")
